...
def get_coin_data(slug: str) -> dict:
    try:
        r = requests.get(
...
def get_bubbles_data(period: str = "24h") -> list:
    """
...
def calc_market_score(fg: dict, global_data: dict, prices: dict) -> tuple:
    """
...
def format_bubbles(coins: list, period: str) -> list:
...
def get_sector_coins(category_id: str, limit: int = 15) -> list:
    """Fetch monede dintr-un sector CoinGecko cu retry."""
    cache_key = f"sector:{category_id}"
    cached = cache_get(cache_key)
    if cached is not None:
        return cached
    for attempt in range(3):
        ...
        try:
            ...
            if r.status_code == 429:
                logger.warning(f"get_sector_coins rate limited, attempt {attempt+1}/3")
                time.sleep(15)
                continue
            if r.status_code == 200:
                result = [
            ...
            logger.error(f"get_sector_coins error (attempt {attempt+1}): {e}")
    return []
